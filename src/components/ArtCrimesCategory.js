...
const ArtCrimesCategory = (props) => {
    ...
    const category = props.wanted

    useEffect(() => {
        getWanted(category, page);
    }, [category, page]);

    const getWanted = async () => {
        const urlFbi = `https://api.fbi.gov/artcrimes/list?crimeCategory=${category}&pageSize=10&page=${page}`;
        ...
        <>
            <div>
                <div className="fbiMain">
                    <div className="paddings"></div>
                    <p className="results">
                        {total} results for {category} in categories
                    </p>
                </div>
...
export default ArtCrimesCategory;